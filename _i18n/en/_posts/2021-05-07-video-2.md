...
* **Device engine and transport module** : Audio engine, video engine, transport module. In the transport module.
    ...
    > Peer-to-peer networks (abbr. __P2P__ ), an Internet system that exchanges information without a central server and relies on groups of users (peers), which serves to reduce the number of nodes in previous network transmissions in order to reduce the risk of data loss.
    > __NAT__ Network Address Translation (NAT) is a technology that rewrites the source or destination IP address of an IP packet as it passes through a router or firewall. It is used to convert internal private IP addresses into public IP addresses that can be used on the public network, initially to solve the problem of a shortage of public IP addresses. The problem. Therefore, in p2p networks, it is necessary to use __NAT__ penetration technology to be able to transmit over the extranet.
